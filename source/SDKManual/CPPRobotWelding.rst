...
.. code-block:: c++
    ...
    if (rtn != 0)
    {
        ...
        std::cout << "============================================================" << std::endl;
    std::cout << " Test velocità e offset tessitura in tempo reale" << std::endl;
    std::cout << "============================================================" << std::endl;
    ExaxisPos epos(0, 0, 0, 0);
    DescPose offset_pos(0, 0, 0, 0, 0, 0);
    ...
    // ---- Step 1: MoveJ al punto iniziale ----
    ...
    // ---- Step 2: MoveJ al punto di ingresso tessitura ----
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    // ---- Step 3: WeaveStart, avvia thread MoveL tessitura ----
    ...
    weaveThread.detach(); // Esecuzione in background
    std::this_thread::sleep_for(std::chrono::milliseconds(500)); // Attendi l'inizio del movimento
    // ---- Step 4: Test velocità (thread principale, MoveL tessitura in background) ----
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(5000));
    // ---- Step 5: Test offset SetWeaveOffsetRT (thread principale, MoveL tessitura in background) ----
    ...
    // ---- Step 6: Attendi il completamento di MoveL tessitura, poi WeaveEnd ----
    std::cout << "\nStep 6: Wait for weave MoveL, then WeaveEnd" << std::endl;
    // Poiché è stato usato detach, attendi che weaveRunning diventi false
    ...
    robot.WeaveEnd(0);
    std::this_thread::sleep_for(std::chrono::milliseconds(500));
    // ---- Step 7: MoveL di ritorno al punto iniziale ----
    ...
    std::cout << "============================================================" << std::endl;
    std::cout << " Test velocità e offset tessitura in tempo reale completato" << std::endl;
    std::cout << "============================================================" << std::endl;
    }